...
const std::string FreeGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString FreeGUI::DEFAULT_WALLET = "~Default";
...
static bool ThreadSafeMessageBox(FreeGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

FreeGUI::FreeGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void FreeGUI::createActions()
{
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and free: URIs)"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    openAction->setStatusTip(tr("Open a free: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Free command-line options").arg(tr(PACKAGE_NAME)));
...
void FreeGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        if(IconsOnly) {
            ...
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/freecointext")));
        }
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* freeLabelLayout = new QVBoxLayout(toolbarWidget);
        freeLabelLayout->addWidget(labelToolbar);
        freeLabelLayout->addWidget(m_toolbar);
        freeLabelLayout->setDirection(QBoxLayout::TopToBottom);
        freeLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Freecoin Market Price"));
        ...
        labelVersionUpdate->setText("<a href=\"https://github.com/FreeProject/Freecoin/releases\">New Wallet Version Available</a>");
        ...
        // Get the latest Freecoin release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/FreeProject/Freecoin/releases";
...
void FreeGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/rvntext")));
        m_toolbar->setMaximumWidth(65);
...
void FreeGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool FreeGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool FreeGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void FreeGUI::setWalletActionsEnabled(bool enabled)
{
...
void FreeGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void FreeGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void FreeGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void FreeGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void FreeGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void FreeGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void FreeGUI::openClicked()
{
...
void FreeGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void FreeGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void FreeGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void FreeGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void FreeGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void FreeGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** RVN START */
void FreeGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void FreeGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void FreeGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void FreeGUI::gotoRestrictedAssetsPage()
{
...
void FreeGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Free network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void FreeGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void FreeGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void FreeGUI::updateHeadersSyncProgressLabel()
{
...
void FreeGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void FreeGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
...
void FreeGUI::changeEvent(QEvent *e)
{
...
void FreeGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void FreeGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void FreeGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "RVN")
        msg += tr("Amount: %1\n").arg(FreeUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(FreeUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void FreeGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void FreeGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool FreeGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool FreeGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void FreeGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void FreeGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void FreeGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void FreeGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void FreeGUI::detectShutdown()
{
...
void FreeGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void FreeGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMnemonic(FreeGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void FreeGUI::subscribeToCoreSignals()
{
...
void FreeGUI::unsubscribeFromCoreSignals()
{
...
void FreeGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void FreeGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<FreeUnits::Unit> units = FreeUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const FreeUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(FreeUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(FreeUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (FreeUnits::Unit u : FreeUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(FreeUnits::name(u)), this);
...
{
    setText(FreeUnits::name(newUnits));
}
...
void FreeGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void FreeGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "FreeGUI::onPriceUnitChange: " + QString::number(newIndex);
...
void FreeGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/FreeProject/Freecoin/releases"));
