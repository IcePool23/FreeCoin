...
#include "freeaddressvalidator.h"
#include "freeunits.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Free address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new FreeAddressEntryValidator(parent));
    widget->setCheckValidator(new FreeAddressCheckValidator(parent));
}
...
bool parseFreeURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no free: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!FreeUnits::parse(FreeUnits::RVN, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseFreeURI(QString uri, SendCoinsRecipient *out)
{
    // Convert free:// to free:
    //
    //    Cannot handle this later, because free:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("free://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "free:");
    }
    QUrl uriInstance(uri);
    return parseFreeURI(uriInstance, out);
}

QString formatFreeURI(const SendCoinsRecipient &info)
{
    QString ret = QString("free:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(FreeUnits::format(FreeUnits::RVN, info.amount, false, FreeUnits::separatorNever));
        paramCount++;
    }
...
bool openFreeConf()
{
    ...
    /* Open free.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Free (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Free (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("free-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a free.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Free\n";
        else
            optionFile << strprintf("Name=Free (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the free app
...
bool GetStartOnSystemStartup()
{
    CFURLRef freeAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (freeAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, freeAppUrl);

    CFRelease(freeAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add free app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, freeAppUrl, nullptr, nullptr);
    }
