 ...
 * between freed, and free-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    FlushWallets();
#endif
    GenerateFrees(false, 0, GetParams());
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/FreeProject/Freecoin>";
    const std::string URL_WEBSITE = "<https://freecoin.org>";
...
/** Sanity checks
 *  Ensure that Free is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Free process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    GenerateFrees(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
