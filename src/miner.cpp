...
void static FreeMiner(const CChainParams& chainparams)
{
    LogPrintf("FreeMiner -- started\n");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("FreeMiner -- Wallet not available\n");
    }
#endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("FreeMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("FreeMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("FreeMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("FreeMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateFrees(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&FreeMiner, boost::cref(chainparams)));
    }
