...
The RPM spec file provided here is for Free-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://free.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Free git repository](https://github.com/FreeProject/Freecoin) but are not part of
...
Currently only `Source10` is used, for the example `free.conf` file.
...
in the free git repository. At present only an SVG version of the Free
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the free
...
needed when 0.12.1 is released, a proper fix is already in the Free git
master branch.
...
idea to build Free Core against a static version of that library compiled
...
The `free.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `free-server` package. At this point, most RPM based
...
The `free-server` package should have SELinux support. How to properly do
...
Free-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/free/). He has been
packaging Free for Fedora at least since 2012.
...
4. I renamed the `free` package that contains the Qt GUI to `free-core` as
that appears to be how the general population refers to it, in contrast to
`free-xt` or `free-classic`. I wanted to make sure the general population
