...
The Free Core daemon can be configured to act as a trusted "border
router", implementing the free wire protocol and relay, making
...
The ZeroMQ feature in Free Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building freed:
...
For instance:
    ...
    $ freed -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/freed.tx.raw
...
These options can also be provided in free.conf.
...
From the perspective of freed, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into freed directly. Furthermore, no information is
...
using. Freed appends an up-counting sequence number to each
