...
dnl Copyright (c) 2021 The Freecoin Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

dnl Helper for cases where a qt dependency is not met.
dnl Output: If qt version is auto, set free_enable_qt to false. Else, exit.
AC_DEFUN([RAVEN_QT_FAIL],[
  if test "x$free_qt_want_version" = xauto && test "x$free_qt_force" != xyes; then
    if test "x$free_enable_qt" != xno; then
      AC_MSG_WARN([$1; free-qt frontend will not be built])
    fi
    free_enable_qt=no
    free_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([RAVEN_QT_CHECK],[
  if test "x$free_enable_qt" != xno && test "x$free_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([RAVEN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build free-qt GUI (default=auto)])],
    [
     free_qt_want_version=$withval
     if test "x$free_qt_want_version" = xyes; then
       free_qt_force=yes
       free_qt_want_version=auto
     fi
    ],
    [free_qt_want_version=auto])
  ...
  AS_IF([test "x$with_gui" = xqt5_debug],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); free_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl Outputs: free_enable_qt, free_enable_qt_dbus, free_enable_qt_test
AC_DEFUN([RAVEN_QT_CONFIGURE],[
  ...
  _RAVEN_QT_IS_STATIC
  if test "x$free_cv_static_qt" = xyes; then
  ...
  RAVEN_QT_CHECK([
    free_enable_qt=yes
    free_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      free_enable_qt_test=no
    fi
    free_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      free_enable_qt_dbus=yes
    fi
  ...
  if test x$free_enable_qt = xyes; then
    AC_MSG_RESULT([$free_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$free_enable_qt])
  fi
...
dnl Output: free_cv_static_qt=yes|no
AC_DEFUN([_RAVEN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, free_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [free_cv_static_qt=yes],
      [free_cv_static_qt=no])
    ])
])
